/**
 * This is a JQuery Formset for Django-Select2 App.
 *
 * jQuery Formset 2.2
...
;(function($) {
    $.fn.formset = function(opts)
    {
                ...
                row.find('a.' + options.deleteCssClass).on('click',function() {
                    ...
                    if (del.length) {
                        ...
                        for (var i=0, formCount=forms.length; i<formCount; i++) {
                            ...
                            forms.eq(i).find('input,select,textarea,label,span,div').each(function() {
        ...
        if ($$.length) {
            ...
            if (options.formTemplate) {
                ...
                template.find('input,select,textarea,label,span,div').each(function() {
            ...
            addButton.on('click', function() {
                ...
                row.find('input,select,textarea,label,span,div').each(function() {
                    updateElementIndex($(this), options.prefix, formCount);
                });
		// Fix the django-select2 code	
                row.find('script').each(function(){
                    if($(this).html()){
                       var code = $(this).html();
                       var idRegex = new RegExp('(' + options.prefix + '-\\d+-)', 'g');
                       var replacement = options.prefix + '-' + formCount + '-';
                       var newcode = code.replace(idRegex, replacement);
    ...
    $.fn.formset.defaults = {
        ...
        addText: '<i class=" fa fa-plus-square fa-2x"></i>',          // Text for the add link
        deleteText: '<i class="fa fa-trash fa-lg"></i>',            // Text for the delete link
